...
/// A fast non-cryptographic 64-bit hash wrapper based on SpookyHash.
///
/// This class provides a convenient interface for incremental hashing using
/// the SpookyHash algorithm.
class FastHash final {
  details::SpookyHash hasher_;

 public:
  /// Constructs a new FastHash instance with a given seed.
  ///
  /// @param seed A 64-bit seed to initialize the hash state.
  FastHash(uint64 seed) { hasher_.Init(seed, 1528); }

  /// Appends data to the hash.
  ///
  /// This method can be called multiple times to hash input data in chunks.
  ///
  ...
  ///
  /// This method finalizes the internal state and returns the first
  /// 64 bits of the 128-bit SpookyHash output.
  ///
  /// @return A 64-bit hash value.
  uint64 final() {
    uint64 res[2];
    hasher_.Final(&res[0], &res[1]);
    return res[0];
  }
};
