...
absl::flat_hash_set<const google::protobuf::Field*> GetRequiredFields(
    const google::protobuf::Type& type) {
  absl::flat_hash_set<const google::protobuf::Field*> required;
...
ProtoWriter::ProtoElement* ProtoWriter::ProtoElement::pop() {
  if (!proto3_) {
    ...
    // seen.
    for (const google::protobuf::Field* r : required_fields_) {
      ow_->MissingField(ow_->use_json_name_in_missing_fields_ ? r->json_name()
                                                              : r->name());
    }
  }
