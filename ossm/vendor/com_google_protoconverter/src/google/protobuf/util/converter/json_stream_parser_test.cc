...
std::string GenerateArrayOfLength(size_t length) {
  return absl::StrCat(std::string(length, '['), "1, 2, 3, 4, 5",
                      std::string(length, ']'));
}
 ...
 protected:
  ...
  absl::Status RunTest(absl::string_view json, int split,
    ...
    if (split == json.length()) {
      ABSL_LOG(INFO) << "Testing split every char: " << json;
    ...
    ABSL_LOG(INFO) << "Testing split: " << first << "><" << rest;
...
TEST_F(JsonStreamParserTest, DeepNestArrayBelowLimit) {
  std::string str = GenerateArrayOfLength(100);
  DoTest(str, 0);
}

TEST_F(JsonStreamParserTest, DeepNestArrayExceedLimit) {
  std::string str = GenerateArrayOfLength(101);
  DoErrorTest(str, 0, "Message too deep. Max recursion depth reached in array");
}
