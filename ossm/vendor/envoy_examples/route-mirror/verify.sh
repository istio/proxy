...
"${DOCKER_COMPOSE[@]}" logs service1-mirror | grep --quiet "Host: localhost-shadow:${PORT_PROXY}"
"${DOCKER_COMPOSE[@]}" logs service1-mirror | grep --quiet GET
...
run_log "View logs for the request mirrored by request header"
"${DOCKER_COMPOSE[@]}" logs service2 | grep --quiet "Host: localhost:${PORT_PROXY}"
"${DOCKER_COMPOSE[@]}" logs service2-mirror | grep --quiet "Host: localhost-shadow:${PORT_PROXY}"
"${DOCKER_COMPOSE[@]}" logs service2-mirror | grep --quiet GET
...
"${DOCKER_COMPOSE[@]}" logs service2-mirror | grep -c GET | grep --quiet 1
