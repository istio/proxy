...
curl -sk --http1.1 "https://localhost:${PORT_PROXY}" | jq  '.hostname' | grep service-https0

run_log "Curl tls inspector: HTTPS -> HTTP/2"
curl -sk --http2 "https://localhost:${PORT_PROXY}" | jq  '.hostname' | grep service-https1

run_log "Curl tls inspector: HTTP"
curl -s "http://localhost:${PORT_PROXY}" | jq  '.hostname' | grep service-http
