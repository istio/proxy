...
load(":apple_configuration.bzl", "apple_configuration")
load(":apple_env.bzl", "apple_host_system_env", "target_apple_env")
...
def _create_compilation_attributes(ctx):
    ...
    return struct(
        ...
        copts = cc_common.objc_expand_and_tokenize(ctx = ctx, attr = "copts", flags = getattr(ctx.attr, "copts", [])),
        conlyopts = cc_common.objc_expand_and_tokenize(ctx = ctx, attr = "conlyopts", flags = getattr(ctx.attr, "conlyopts", [])),
...
def _compile(
        ...
        extension,
        extra_compile_args,
        priority_headers,  # buildifier: disable=unused-variable
        srcs,
        private_hdrs,
        public_hdrs,
        pch_hdr,
...
def _register_binary_strip_action(
    ...
    apple_config = apple_common.get_apple_config(config = build_config)
...
def _register_configuration_specific_link_actions_with_cpp_variables(
        ...
        user_variable_extensions,
        additional_outputs,
        deps,  # buildifier: disable=unused-variable
        extra_link_inputs,
        attr_linkopts):
    ctx = common_variables.ctx
...
def _create_deduped_linkopts_linking_context(
        owner,  # buildifier: disable=unused-variable
        cc_linking_context,
        seen_flags):
    linker_inputs = []
