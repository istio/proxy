...
def _compute_public_headers(
    ...
    if strip_include_prefix:
        ...
        if paths.is_absolute(strip_prefix):
            ...
            # is "." get_relative_path("", ".") returns "." instead of "".
            strip_prefix = get_relative_path(label.package, strip_prefix)
            if strip_prefix == ".":
                strip_prefix = ""
    ...
    for original_header in public_headers_artifacts:
        ...
        if include_prefix != None:
            include_path = get_relative_path(include_prefix, include_path)
