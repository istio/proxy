/*
** server.c
 * Demo server to check that we can extract all parameters from an incoming request
 * and respond to it
*/
...
#define DEFAULT_PORT "3490"  // the port users will be connecting to, if not specified on command line
...
int main(int argc, char* argv[])
{
    ...
    inet_ntop(p->ai_family, &bind_sock_addr->sin_addr, bind_addr_str, p->ai_addrlen);
    printf("server %s: waiting for connections on port %s:%u...\n",
           port, bind_addr_str, ntohs(bind_sock_addr->sin_port));
    ...
    while(1) {
        ...
        inet_ntop(their_addr.ss_family, &src_sock_addr->sin_addr, their_addr_str, addr_str_len);
        printf("server %s: got connection FROM %s:%u\n", port, their_addr_str, ntohs(src_sock_addr->sin_port));
        ...
        inet_ntop(my_addr.ss_family, &dst_sock_addr->sin_addr, my_addr_str, addr_str_len);
        printf("server %s: got connection TO %s:%u\n", port, my_addr_str, ntohs(dst_sock_addr->sin_port));
        ...
        int status = getsockopt(accepted_fd, SOL_IP, SO_ORIGINAL_DST, orig_sock_addr, &addr_len);

        if (status == 0) {
            inet_ntop(orig_addr.ss_family, &orig_sock_addr->sin_addr, orig_addr_str, addr_str_len);
            printf("server %s: ORIG DEST %s:%u\n", port, orig_addr_str, ntohs(orig_sock_addr->sin_port));
        }
        else
        {
        ...
        if (!fork()) { // this is the child process
            ...
            snprintf(msg, 256, "FROM %s:%u, TO %s:%u, ORIG DEST %s:%u\n",
                     their_addr_str, ntohs(src_sock_addr->sin_port),
                     my_addr_str, ntohs(dst_sock_addr->sin_port),
                     orig_addr_str, ntohs(orig_sock_addr->sin_port));
