...
ApiManagerImpl::ApiManagerImpl(std::unique_ptr<ApiManagerEnvInterface> env,
    ...
    : input_(input),
      global_context_(
          new context::GlobalContext(std::move(env), input.server_config)) {
  if (!input.metadata_server.empty()) {
    global_context_->SetMetadataServer(input.metadata_server);
  }
  if (!input.google_authentication_secret.empty()) {
    global_context_->service_account_token()->SetClientAuthSecret(
        input.google_authentication_secret);
  }
  if (!input.service_config.empty()) {
    AddConfig(input.service_config);
  }
...
std::shared_ptr<ApiManager> ApiManagerFactory::CreateApiManager(
    std::unique_ptr<ApiManagerEnvInterface> env, const ApiManagerInput &input) {
  return std::shared_ptr<ApiManager>(new ApiManagerImpl(std::move(env), input));
}
