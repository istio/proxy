...
// The input data to create ApiManager object.
struct ApiManagerInput {
  // The buffer to hold the whole service config.
  std::string service_config;

  // The server config.
  std::string server_config;

  // The metadata server for GCP platforms.
  std::string metadata_server;

  // Client secret to generate tokens for authenticating against
  // Google services like service-control, cloud-trace API, etc
  std::string google_authentication_secret;

  // service name
  std::string service_name;

  enum RolloutStrategy {
    // Jus use the service config at start of ESP, not to follow
    // the service management service config rollout.
    FIXED = 0,
    // Follow the service management service config rollout.
    MANAGED,
  };
  RolloutStrategy rollout_strategy;

  // Use this service config at start of ESP.
