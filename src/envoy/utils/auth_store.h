...
class JwtAuthStore : public ThreadLocal::ThreadLocalObject {
 public:
  ...
  JwtAuthStore(JwtTokenExtractor::RuleSet_t& config)
      : config_(config), pubkey_cache_(config_), token_extractor_(config_) {}

  // Get the Config.
  JwtTokenExtractor::RuleSet_t config() const {
    return config_;
  }
 ...
 private:
  // Store the config.
  JwtTokenExtractor::RuleSet_t& config_;
...
class JwtAuthStoreFactory : public Logger::Loggable<Logger::Id::config> {
 public:
  JwtAuthStoreFactory(JwtTokenExtractor::RuleSet_t& config,
