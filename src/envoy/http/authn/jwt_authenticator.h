...
class JwtAuthenticator : public Logger::Loggable<Logger::Id::filter>,
 ...
 public:
  JwtAuthenticator(Upstream::ClusterManager& cm, JwtAuthnStore& store);

  // The callback interface to notify the completion event.
  class Callbacks {
   public:
    virtual ~Callbacks() {}
    virtual void onDone(const JwtAuth::Status& status) PURE;
  };
 ...
 private:
  ...
  void FetchPubkey(JwtPubkeyCacheItem* issuer);
  ...
  void DoneWithStatus(const JwtAuth::Status& status);

  // The cluster manager object to make HTTP call.
  Upstream::ClusterManager& cm_;
  // The JWT cache object.
  JwtAuthnStore& store_;
  // The JWT object.
