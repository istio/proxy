...
#include "src/envoy/http/authn/jwt_authenticator.h"
...
void JwtAuthenticator::Verify(HeaderMap& headers,
  ...
  if (!entry) {
    DoneWithStatus(JwtAuth::Status::JWT_MISSED);
    return;
  }
  ...
  if (!StringUtil::startsWith(value.c_str(), kBearerPrefix, true)) {
    DoneWithStatus(JwtAuth::Status::BEARER_PREFIX_MISMATCH);
    return;
  }

  // Parse JWT token
  jwt_.reset(new JwtAuth::Jwt(value.c_str() + kBearerPrefix.length()));
  if (jwt_->GetStatus() != JwtAuth::Status::OK) {
  ...
  if (jwt_->Exp() < unix_timestamp) {
    DoneWithStatus(JwtAuth::Status::JWT_EXPIRED);
    return;
  }
  ...
  if (!issuer) {
    DoneWithStatus(JwtAuth::Status::JWT_UNKNOWN_ISSUER);
    return;
  }

  // Check if audience is allowed
  if (!issuer->IsAudienceAllowed(jwt_->Aud())) {
    DoneWithStatus(JwtAuth::Status::AUDIENCE_NOT_ALLOWED);
    return;
  }
...
void JwtAuthenticator::FetchPubkey(JwtPubkeyCacheItem* issuer) {
  // Lei: as a POC, uri_ is hardcoded
  // Todo: pass uri_ from the config
  if (issuer == nullptr) {
    return;
  }
  uri_ = "http://localhost:8081/";
  ...
  // Lei: as a POC, cluster is hardcoded
  // Todo: add cluster to the config
  const auto& cluster = "example_issuer";
  // const auto& cluster = issuer->jwt_config().jwks_uri_envoy_cluster();
  if (cm_.get(cluster) == nullptr) {
    DoneWithStatus(JwtAuth::Status::FAILED_FETCH_PUBKEY);
    return;
  }
...
void JwtAuthenticator::OnFetchPubkeyDone(const std::string& pubkey) {
  auto issuer = store_.pubkey_cache().LookupByIssuer(jwt_->Iss());
  JwtAuth::Status status = issuer->SetKey(pubkey);
...
void JwtAuthenticator::VerifyKey(const JwtAuth::Pubkeys& pubkey) {
  ...
  DoneWithStatus(JwtAuth::Status::OK);
}

void JwtAuthenticator::DoneWithStatus(const JwtAuth::Status& status) {
