...
/**
 * All memory for the upstream policy and quota checks should hang off of these objects.
 */
class CheckContext : public CheckResponseInfo {
 public:
  ...
  const istio::mixer::v1::Attributes* attributes() const { return shared_attributes_->attributes(); }

  const std::vector<istio::quota_config::Requirement>& quotaRequirements() const { return quota_requirements_; }
  std::vector<istio::quota_config::Requirement>& quotaRequirements() { return quota_requirements_; }
  ...
  const google::protobuf::util::Status& policyStatus() const { return policy_cache_result_.status(); }
  ...
  void updatePolicyCache(const google::protobuf::util::Status& status,
    ...
    policy_cache_result_.SetResponse(status, *shared_attributes_->attributes(), response);
  }
  ...
  bool remoteQuotaRequestRequired() const { return remote_quota_check_required_; }
  ...
  void checkQuotaCache(QuotaCache& quotaCache) {
    ...
    //
    // Quota is removed from the quota cache iff there is a policy cache hit.   If there is a policy cache miss, then a
    // request has to be sent upstream anyways, so the quota will be decremented on the upstream response.
    //
    quotaCache.Check(*shared_attributes_->attributes(), quota_requirements_, policyCacheHit(), &quota_cache_result_);

    remote_quota_check_required_ = quota_cache_result_.BuildRequest(allocRequestOnce());
  ...
  void updateQuotaCache(const google::protobuf::util::Status& status, const istio::mixer::v1::CheckResponse& response) {
    quota_cache_result_.SetResponse(status, *shared_attributes_->attributes(), response);
  }

  bool quotaCacheHit() const { return quota_cache_hit_; }
  const google::protobuf::util::Status& quotaStatus() const { return quota_cache_result_.status(); }

  //
  // Upstream request and response
  //

  void compressRequest(const AttributeCompressor& compressor, const std::string &deduplication_id) {
    compressor.Compress(*shared_attributes_->attributes(), allocRequestOnce()->mutable_attributes());
  ...
  istio::mixer::v1::CheckResponse* response() {
    if (!response_) {
      response_ = google::protobuf::Arena::CreateMessage<istio::mixer::v1::CheckResponse>(&shared_attributes_->arena());
    }
    return response_;
  }

  void setFinalStatus(const google::protobuf::util::Status& status) { final_status_ = status; }

  //
  // CheckResponseInfo (exposed to the top-level filter)
  //

  virtual const google::protobuf::util::Status& status() const override { return final_status_; }

  virtual const istio::mixer::v1::RouteDirective& routeDirective() const override {
 ...
 private:
  ...
  istio::mixer::v1::CheckRequest* allocRequestOnce() {
    if (!request_) {
      request_ = google::protobuf::Arena::CreateMessage<istio::mixer::v1::CheckRequest>(&shared_attributes_->arena());
    }
