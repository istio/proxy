...
static std::atomic<uint32_t> REMOTE_REQUEST_FAIL_LOG_MESSAGES{0};
static constexpr uint32_t REMOTE_REQUEST_FAIL_LOG_MODULUS{100};
...
CancelFunc MixerClientImpl::Check(
  ...
  return transport(
      ...
      [this, context = context, on_done](const Status &status) {
        ...
        if (TransportResult::SUCCESS != result && MIXER_WARN_ENABLED &&
            0 == REMOTE_REQUEST_FAIL_LOG_MESSAGES++ %
                     REMOTE_REQUEST_FAIL_LOG_MODULUS) {
          MIXER_WARN("Policy check failed. transport=%s, policy=%s, quota=%s",
                     status.ToString().c_str(),
                     result == TransportResult::SUCCESS
                         ? context->policyStatus().ToString().c_str()
                         : "NA",
                     result == TransportResult::SUCCESS &&
                             context->quotaCheckRequired()
                         ? context->policyStatus().ToString().c_str()
                         : "NA");
        } else if (MIXER_DEBUG_ENABLED) {
          MIXER_DEBUG("CheckResult transport=%s, policy=%s, quota=%s",
                      status.ToString().c_str(),
                      result == TransportResult::SUCCESS
                          ? context->policyStatus().ToString().c_str()
                          : "NA",
...
void MixerClientImpl::Report(istio::mixerclient::SharedAttributesPtr &context) {
