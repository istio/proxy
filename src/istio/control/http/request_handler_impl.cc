...
RequestHandlerImpl::RequestHandlerImpl(std::shared_ptr<ServiceContext> service_context)
    : attributes_(new istio::mixerclient::SharedAttributes()),
      check_context_(new istio::mixerclient::CheckContext(
          service_context->client_context()->NetworkFailOpen(),
          attributes_)),
...
void RequestHandlerImpl::AddCheckAttributes(CheckData* check_data) {
  ...
  if (service_context_->enable_mixer_check() || service_context_->enable_mixer_report()) {
...
CancelFunc RequestHandlerImpl::Check(CheckData* check_data, HeaderUpdate* header_update, TransportCheckFunc transport,
  ...
  if (!service_context_->enable_mixer_check()) {
    check_context_->setFinalStatus(Status::OK);
...
void RequestHandlerImpl::Report(CheckData* check_data, ReportData* report_data) {
  ...
  AttributesBuilder builder(attributes_->attributes());
  builder.ExtractReportAttributes(check_context_->policyStatus(), report_data);
