...
CancelFunc ClientContextBase::SendCheck(TransportCheckFunc transport,
  ...
  if (MIXER_DEBUG_ENABLED) {
    MIXER_DEBUG("Check attributes: %s", context->attributes()->DebugString().c_str());
  }

  return mixer_client_->Check(context, transport, on_done);
}

void ClientContextBase::SendReport(istio::mixerclient::SharedAttributesPtr& attributes) {
  if (MIXER_DEBUG_ENABLED) {
    MIXER_DEBUG("Report attributes: %s", attributes->attributes()->DebugString().c_str());
  }
