...
bool AuthnUtils::ProcessJwtPayload(const std::string& payload_str,
  ...
  try {
    ...
    logDebug(absl::StrCat("{}: json object is {}", __FUNCTION__,
...
bool AuthnUtils::ExtractOriginalPayload(const std::string& token,
  ...
  try {
    ...
    *original_payload = original_payload_obj->asJsonString();
    logDebug("{}: the original payload in exchanged token is {}", __FUNCTION__, *original_payload);
  } catch (...) {
    logDebug("{}: original_payload in exchanged token is of invalid format.",
              __FUNCTION__);
    return false;
  }
