...
void instanceFromMetadata(const ::wasm::common::NodeInfo& node_info,
  ...
  if ((!node_info.name().empty()) && (!node_info.namespace_().empty())) {
    absl::StrAppend(instance->mutable_uid(), kKubeUIDPrefix, node_info.name(),
...
EdgeReporter::~EdgeReporter() {
  ...
  //   logWarn("EdgeReporter had uncommitted TrafficAssertions when
  //   shutdown.");
  // }
}
