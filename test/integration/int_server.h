...
class ServerStream {
 public:
   ...
   * @param delay delay in msec before sending the response.  if 0 send immediately
   */
  virtual void sendGrpcResponse(Envoy::Grpc::Status::GrpcStatus status, const Envoy::Protobuf::Message &response,
                                const std::chrono::milliseconds delay = std::chrono::milliseconds(0)) PURE;
...
// NB: references passed to any of these callbacks are owned by the caller and must not be used after the callback
// returns -- except for the request headers which may be moved into the caller.
typedef std::function<ServerCallbackResult(ServerConnection &server_connection)> ServerAcceptCallback;
typedef std::function<void(ServerConnection &connection, ServerCloseReason reason)> ServerCloseCallback;
...
class ServerFilterChain : public Envoy::Network::FilterChain {
 public:
  ...
  virtual const std::vector<Envoy::Network::FilterFactoryCb> &networkFilterFactories() const override;
...
class LocalListenSocket : public Envoy::Network::TcpListenSocket {
 public:
   ...
   * @param bind_to_port if true immediately bind to the port, allocating one if necessary.  true by default.
   */
  LocalListenSocket(Envoy::Network::Address::IpVersion ip_version = Envoy::Network::Address::IpVersion::v4,
...
/**
 * A convenience class for passing callbacks to a Server.  If no callbacks are provided, default callbacks that track
 * some simple metrics will be used.   If callbacks are provided, they will be wrapped with callbacks that maintain the
 * same simple set of metrics.
 */
class ServerCallbackHelper {
 public:
  ServerCallbackHelper(ServerRequestCallback request_callback = nullptr, ServerAcceptCallback accept_callback = nullptr,
  ...
  /*
   * Wait until the server has accepted n connections and seen them closed (due to error or client close)
   */
...
class Server : public Envoy::Network::FilterChainManager,
 ...
 public:
  ...
  Server(const std::string &name, Envoy::Network::Socket &listening_socket,
         Envoy::Network::TransportSocketFactory &transport_socket_factory, Envoy::Http::CodecClient::Type http_type);
  ...
  virtual const Envoy::Network::FilterChain *findFilterChain(const Envoy::Network::ConnectionSocket &) const override;

  //
  // Envoy::Network::FilterChainFactory
  //

  virtual bool createNetworkFilterChain(Envoy::Network::Connection &network_connection,
