...
class ServerStreamImpl : public ServerStream,
 ...
 public:
  ServerStreamImpl(uint32_t id, ServerConnection &connection, ServerRequestCallback request_callback,
                   Envoy::Http::StreamEncoder &stream_encoder)
      : id_(id), connection_(connection), request_callback_(request_callback), stream_encoder_(stream_encoder) {
  ...
  virtual void sendResponseHeaders(const Envoy::Http::HeaderMap &response_headers,
    ...
    if (connection_.networkConnection().state() != Envoy::Network::Connection::State::Open) {
      ENVOY_LOG(warn, "ServerStream({}:{}:{})'s underlying connection is not open!", connection_.name(),
                connection_.id(), id_);
    ...
    if (delay <= std::chrono::milliseconds(0)) {
      ENVOY_LOG(debug, "ServerStream({}:{}:{}) sending response headers", connection_.name(), connection_.id(), id_);
    ...
    delay_timer = connection_.dispatcher().createTimer([this, delay]() {
      ENVOY_LOG(debug, "ServerStream({}:{}:{}) sending response headers after {} msec delay", connection_.name(),
                connection_.id(), id_, static_cast<long int>(delay.count()));
  ...
  virtual void sendGrpcResponse(Envoy::Grpc::Status::GrpcStatus status, const Envoy::Protobuf::Message &message,
                                const std::chrono::milliseconds delay) override {
    if (delay <= std::chrono::milliseconds(0)) {
      ENVOY_LOG(debug, "ServerStream({}:{}:{}) sending gRPC response", connection_.name(), connection_.id(), id_);
      ...
      stream_encoder_.encodeTrailers(
          Envoy::Http::TestHeaderMapImpl{{"grpc-status", std::to_string(static_cast<uint32_t>(status))}});
    ...
    delay_timer = connection_.dispatcher().createTimer([this, delay]() {
      ENVOY_LOG(debug, "ServerStream({}:{}:{}) sending gRPC response after {} msec delay", connection_.name(),
                connection_.id(), id_, static_cast<long int>(delay.count()));
      stream_encoder_.encodeHeaders(Envoy::Http::TestHeaderMapImpl{{":status", "200"}}, false);
      stream_encoder_.encodeData(*response_body_, false);
      stream_encoder_.encodeTrailers(
          Envoy::Http::TestHeaderMapImpl{{"grpc-status", std::to_string(static_cast<uint32_t>(response_status_))}});
      delay_timer->disableTimer();
  ...
  virtual void decode100ContinueHeaders(Envoy::Http::HeaderMapPtr &&) override {
    ENVOY_LOG(error, "ServerStream({}:{}:{}) got continue headers?!?!", connection_.name(), connection_.id(), id_);
  }
  ...
  virtual void decodeHeaders(Envoy::Http::HeaderMapPtr &&headers, bool end_stream) override {
    ENVOY_LOG(debug, "ServerStream({}:{}:{}) got request headers", connection_.name(), connection_.id(), id_);
  ...
  virtual void decodeData(Envoy::Buffer::Instance &, bool end_stream) override {
    ENVOY_LOG(debug, "ServerStream({}:{}:{}) got request body data", connection_.name(), connection_.id(), id_);
  ...
  virtual void decodeTrailers(Envoy::Http::HeaderMapPtr &&) override {
    ENVOY_LOG(trace, "ServerStream({}:{}:{}) got request trailers", connection_.name(), connection_.id(), id_);
    onEndStream();
    // stream is now destroyed
  }

  virtual void decodeMetadata(Envoy::Http::MetadataMapPtr &&) override {
    ENVOY_LOG(trace, "ServerStream({}:{}):{} got metadata", connection_.name(), connection_.id(), id_);
  }
  ...
  virtual void onResetStream(Envoy::Http::StreamResetReason reason) override {
    ...
    switch (reason) {
      case Envoy::Http::StreamResetReason::LocalReset:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) was locally reset", connection_.name(), connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::LocalRefusedStreamReset:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) refused local stream reset", connection_.name(), connection_.id(),
                  id_);
        break;
      case Envoy::Http::StreamResetReason::RemoteReset:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) was remotely reset", connection_.name(), connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::RemoteRefusedStreamReset:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) refused remote stream reset", connection_.name(), connection_.id(),
                  id_);
        break;
      case Envoy::Http::StreamResetReason::ConnectionFailure:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) reseet due to initial connection failure", connection_.name(),
                  connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::ConnectionTermination:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) reset due to underlying connection reset", connection_.name(),
                  connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::Overflow:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) reset due to resource overflow", connection_.name(), connection_.id(),
                  id_);
        break;
      default:
        ENVOY_LOG(trace, "ServerStream({}:{}:{}) reset due to unknown reason", connection_.name(), connection_.id(),
                  id_);
        break;
    }
  }
  ...
  virtual void onAboveWriteBufferHighWatermark() override {
    ...
    ENVOY_LOG(trace, "ServerStream({}:{}:{}) above write buffer high watermark", connection_.name(), connection_.id(),
              id_);
  }

  virtual void onBelowWriteBufferLowWatermark() override {
    // TODO is their anything to be done here?
    ENVOY_LOG(trace, "ServerStream({}:{}:{}) below write buffer low watermark", connection_.name(), connection_.id(),
              id_);
  }
...
ServerConnection::ServerConnection(const std::string &name, uint32_t id, ServerRequestCallback request_callback,
                                   ServerCloseCallback close_callback, Envoy::Network::Connection &network_connection,
  ...
  switch (http_type) {
    case Envoy::Http::CodecClient::Type::HTTP1:
      http_connection_ = std::make_unique<Envoy::Http::Http1::ServerConnectionImpl>(network_connection, *this,
    ...
    case Envoy::Http::CodecClient::Type::HTTP2: {
      ...
      http_connection_ =
          std::make_unique<Envoy::Http::Http2::ServerConnectionImpl>(network_connection, *this, scope, settings);
    } break;
    default:
      ENVOY_LOG(error, "ServerConnection({}:{}) doesn't support http type %d, defaulting to HTTP1", name_, id_,
                static_cast<int>(http_type) + 1);
      http_connection_ = std::make_unique<Envoy::Http::Http1::ServerConnectionImpl>(network_connection, *this,
                                                                                    Envoy::Http::Http1Settings());
  }
}

ServerConnection::~ServerConnection() { ENVOY_LOG(trace, "ServerConnection({}:{}) destroyed", name_, id_); }
...
const Envoy::Network::Connection &ServerConnection::networkConnection() const { return network_connection_; }

Envoy::Http::ServerConnection &ServerConnection::httpConnection() { return *http_connection_; }

const Envoy::Http::ServerConnection &ServerConnection::httpConnection() const { return *http_connection_; }

Envoy::Event::Dispatcher &ServerConnection::dispatcher() { return dispatcher_; }

Envoy::Network::FilterStatus ServerConnection::onData(Envoy::Buffer::Instance &data, bool end_stream) {
  ...
  try {
    ...
    ENVOY_LOG(error, "ServerConnection({}:{}) received the wrong protocol: {}", name_, id_, e.what());
    network_connection_.close(Envoy::Network::ConnectionCloseType::NoFlush);
    return Envoy::Network::FilterStatus::StopIteration;
  }

  if (end_stream) {
    ENVOY_LOG(error, "ServerConnection({}:{}) got end stream - TODO relay to all active streams?!?", name_, id_);
  }
...
Envoy::Http::StreamDecoder &ServerConnection::newStream(Envoy::Http::StreamEncoder &stream_encoder, bool) {
  ...
  ENVOY_LOG(debug, "ServerConnection({}:{}) received new Stream({}:{}:{})", name_, id_, name_, id_, id);
...
ServerFilterChain::ServerFilterChain(Envoy::Network::TransportSocketFactory &transport_socket_factory)
...
const std::vector<Envoy::Network::FilterFactoryCb> &ServerFilterChain::networkFilterFactories() const {
...
LocalListenSocket::LocalListenSocket(Envoy::Network::Address::IpVersion ip_version, uint16_t port,
    ...
    : NetworkListenSocket(
          Envoy::Network::Utility::parseInternetAddress(Envoy::Network::Test::getAnyAddressUrlString(ip_version), port),
          options, bind_to_port) {}

LocalListenSocket::~LocalListenSocket() {}

ServerCallbackHelper::ServerCallbackHelper(ServerRequestCallback request_callback, ServerAcceptCallback accept_callback,
                                           ServerCloseCallback close_callback)
    : accept_callback_(accept_callback), request_callback_(request_callback), close_callback_(close_callback) {
  if (request_callback) {
    request_callback_ = [this, &request_callback](ServerConnection &connection, ServerStream &stream,
                                                  Envoy::Http::HeaderMapPtr request_headers) {
      ++requests_received_;
      request_callback(connection, stream, std::move(request_headers));
    };
  } else {
    request_callback_ = [this](ServerConnection &, ServerStream &stream, Envoy::Http::HeaderMapPtr &&) {
      ++requests_received_;
  ...
  if (accept_callback) {
    accept_callback_ = [this, &accept_callback](ServerConnection &connection) -> ServerCallbackResult {
      ++accepts_;
  ...
  if (close_callback) {
    close_callback_ = [this, &close_callback](ServerConnection &connection, ServerCloseReason reason) {
      switch (reason) {
...
Server::Server(const std::string &name, Envoy::Network::Socket &listening_socket,
    ...
    : name_(name),
      stats_(),
      time_system_(),
      api_(std::chrono::milliseconds(1), Envoy::Thread::ThreadFactorySingleton::get(), stats_),
...
void Server::setPerConnectionBufferLimitBytes(uint32_t limit) { connection_buffer_limit_bytes_ = limit; }
...
std::chrono::milliseconds Server::listenerFiltersTimeout() const { return std::chrono::milliseconds(0); }
...
const Envoy::Network::FilterChain *Server::findFilterChain(const Envoy::Network::ConnectionSocket &) const {
...
bool Server::createNetworkFilterChain(Envoy::Network::Connection &network_connection,
  ...
  ServerConnectionSharedPtr connection = std::make_shared<ServerConnection>(
      name_, id, request_callback_, close_callback_, network_connection, *dispatcher_, http_type_, stats_);
...
const std::vector<ServerCallbackHelperPtr> &ClusterHelper::servers() const { return server_callback_helpers_; }
