 ...
 */
typedef std::function<ClientCallbackResult(ClientConnection &connection, ClientConnectionState state)>
 ...
 */
typedef std::function<void(ClientConnection &connection, ClientCloseReason reason)> ClientCloseCallback;
 ...
 */
typedef std::function<void(ClientConnection &connection, Envoy::Http::HeaderMapPtr response_headers)>
    ClientResponseCallback;

class ClientConnection : public Envoy::Network::ConnectionCallbacks,
 ...
 public:
   ...
   * @param callback
   */
  virtual void sendRequest(const Envoy::Http::HeaderMap &request_headers, ClientResponseCallback callback,
                           const std::chrono::milliseconds timeout = std::chrono::milliseconds(5'000));
...
class Client : Envoy::Logger::Loggable<Envoy::Logger::Id::testing> {
 public:
  ...
  /**
   * Start the client's dispatcher in a background thread.  This is a noop if the client has already been started.  This
   * will block until the dispatcher is running on another thread.
   */
  void start();

  /**
   * Stop the client's dispatcher and join the background thread.  This will block until the background thread exits.
   */
  void stop();
   ...
   * @param callback
   */
  void connect(Envoy::Network::TransportSocketFactory &socket_factory, HttpVersion http_version,
...
class LoadGenerator : Envoy::Logger::Loggable<Envoy::Logger::Id::testing> {
 public:
  /**
   *  A wrapper around Client and its callbacks that implements a simple load generator.
   *
   ...
   * @param sockopts Socket options for the client sockets.  Use default if null.
   */
  LoadGenerator(Client &client, Envoy::Network::TransportSocketFactory &socket_factory, HttpVersion http_version,
                Envoy::Network::Address::InstanceConstSharedPtr &address,
                const Envoy::Network::ConnectionSocket::OptionsSharedPtr &sockopts = nullptr);

  virtual ~LoadGenerator();

  /**
   * Generate load and block until all connections have finished (successfully or otherwise).
   *
   * @param connections Connections to create
   * @param requests Total requests across all connections to send
   * @param request The request to send
   * @param timeout The time in msec to wait to receive a response after sending each request.
   */
  void run(uint32_t connections, uint32_t requests, Envoy::Http::HeaderMapPtr request,
           const std::chrono::milliseconds timeout = std::chrono::milliseconds(5'000));
