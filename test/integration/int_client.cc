...
class ClientStream : public Envoy::Http::StreamDecoder,
 ...
 public:
  ...
  virtual void decode100ContinueHeaders(Envoy::Http::HeaderMapPtr &&) override {
    ENVOY_LOG(trace, "ClientStream({}:{}:{}) got continue headers", connection_.name(), connection_.id(), id_);
  }

  virtual void decodeHeaders(Envoy::Http::HeaderMapPtr &&response_headers, bool end_stream) override {
    ENVOY_LOG(debug, "ClientStream({}:{}:{}) got response headers", connection_.name(), connection_.id(), id_);
  ...
  virtual void decodeData(Envoy::Buffer::Instance &, bool end_stream) override {
    ENVOY_LOG(debug, "ClientStream({}:{}:{}) got response body data", connection_.name(), connection_.id(), id_);
  ...
  virtual void decodeTrailers(Envoy::Http::HeaderMapPtr &&) override {
    ENVOY_LOG(trace, "ClientStream({}:{}:{}) got response trailers", connection_.name(), connection_.id(), id_);
    onEndStream();
    // stream is now destroyed
  }

  virtual void decodeMetadata(Envoy::Http::MetadataMapPtr &&) override {
    ENVOY_LOG(trace, "ClientStream({}:{}):{} got metadata", connection_.name(), connection_.id(), id_);
  }
  ...
  virtual void onResetStream(Envoy::Http::StreamResetReason reason) override {
    // TODO test with h2 to see if we get any of these and whether the connection error handling is enough to handle it.
    switch (reason) {
      case Envoy::Http::StreamResetReason::LocalReset:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) was locally reset", connection_.name(), connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::LocalRefusedStreamReset:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) refused local stream reset", connection_.name(), connection_.id(),
                  id_);
        break;
      case Envoy::Http::StreamResetReason::RemoteReset:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) was remotely reset", connection_.name(), connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::RemoteRefusedStreamReset:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) refused remote stream reset", connection_.name(), connection_.id(),
                  id_);
        break;
      case Envoy::Http::StreamResetReason::ConnectionFailure:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) reseet due to initial connection failure", connection_.name(),
                  connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::ConnectionTermination:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) reset due to underlying connection reset", connection_.name(),
                  connection_.id(), id_);
        break;
      case Envoy::Http::StreamResetReason::Overflow:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) reset due to resource overflow", connection_.name(), connection_.id(),
                  id_);
        break;
      default:
        ENVOY_LOG(trace, "ClientStream({}:{}:{}) reset due to unknown reason", connection_.name(), connection_.id(),
                  id_);
        break;
    }
  }
  ...
  virtual void onAboveWriteBufferHighWatermark() override {
    ...
    ENVOY_LOG(trace, "ClientStream({}:{}:{}) above write buffer high watermark", connection_.name(), connection_.id(),
              id_);
  }

  virtual void onBelowWriteBufferLowWatermark() override {
    // TODO how should this be handled?
    ENVOY_LOG(trace, "ClientStream({}:{}:{}) below write buffer low watermark", connection_.name(), connection_.id(),
              id_);
  }

  virtual void sendRequest(const Envoy::Http::HeaderMap &request_headers, const std::chrono::milliseconds timeout) {
    if (connection_.networkConnection().state() != Envoy::Network::Connection::State::Open) {
      ENVOY_LOG(warn, "ClientStream({}:{}:{})'s underlying connection is not open!", connection_.name(),
                connection_.id(), id_);
    ...
    Envoy::Http::StreamEncoder &encoder = connection_.httpConnection().newStream(*this);
    encoder.getStream().addCallbacks(*this);

    ENVOY_LOG(debug, "ClientStream({}:{}:{}) sending request headers", connection_.name(), connection_.id(), id_);
    encoder.encodeHeaders(request_headers, true);

    timeout_timer_ = connection_.dispatcher().createTimer([this, timeout]() {
      ENVOY_LOG(debug, "ClientStream({}:{}:{}) timed out after {} msec waiting for response", connection_.name(),
                connection_.id(), id_, static_cast<long int>(timeout.count()));
...
class HttpClientReadFilter : public Envoy::Network::ReadFilter, Envoy::Logger::Loggable<Envoy::Logger::Id::testing> {
 public:
  HttpClientReadFilter(const std::string name, uint32_t id, Envoy::Http::ClientConnection &connection)
  ...
  virtual Envoy::Network::FilterStatus onData(Envoy::Buffer::Instance &data, bool end_stream) override {
  ...
  virtual Envoy::Network::FilterStatus onNewConnection() override { return Envoy::Network::FilterStatus::Continue; }
...
class Http1ClientConnection : public ClientConnection {
 public:
  Http1ClientConnection(Client &client, uint32_t id, ClientConnectCallback connect_callback,
                        ClientCloseCallback close_callback, std::shared_ptr<Envoy::Event::Dispatcher> &dispatcher,
  ...
  virtual Envoy::Network::ClientConnection &networkConnection() override { return *network_connection_; }

  virtual Envoy::Http::ClientConnection &httpConnection() override { return http_connection_; }
...
ClientConnection::ClientConnection(Client &client, uint32_t id, ClientConnectCallback connect_callback,
...
ClientConnection::~ClientConnection() { ENVOY_LOG(trace, "ClientConnection({}:{}) destroyed", client_.name(), id_); }

const std::string &ClientConnection::name() const { return client_.name(); }

uint32_t ClientConnection::id() const { return id_; }

Envoy::Event::Dispatcher &ClientConnection::dispatcher() { return *dispatcher_; };
...
void ClientConnection::sendRequest(const Envoy::Http::HeaderMap &headers, ClientResponseCallback callback,
...
void Client::connect(Envoy::Network::TransportSocketFactory &socket_factory, HttpVersion http_version,
  ...
  dispatcher_->post([this, &socket_factory, http_version, address, sockopts, connect_cb, close_cb]() {
    Envoy::Network::ClientConnectionPtr connection =
        dispatcher_->createClientConnection(address, nullptr, socket_factory.createTransportSocket(nullptr), sockopts);
    uint32_t id = connection_counter_++;

    ClientConnectionPtr ptr;
    if (HttpVersion::HTTP1 == http_version) {
      ptr =
          std::make_unique<Http1ClientConnection>(*this, id, connect_cb, close_cb, dispatcher_, std::move(connection));
    } else {
      ptr =
          std::make_unique<Http2ClientConnection>(*this, id, connect_cb, close_cb, dispatcher_, std::move(connection));
    }
...
LoadGenerator::LoadGenerator(Client &client, Envoy::Network::TransportSocketFactory &socket_factory,
                             HttpVersion http_version, Envoy::Network::Address::InstanceConstSharedPtr &address,
                             const Envoy::Network::ConnectionSocket::OptionsSharedPtr &sockopts)
    : client_(client),
      ...
      address_(address),
      sockopts_(sockopts) {
  response_callback_ = [this](ClientConnection &connection, Envoy::Http::HeaderMapPtr response) {
    if (!response) {
      ENVOY_LOG(debug, "Connection({}:{}) timedout waiting for response", connection.name(), connection.id());
      ++response_timeouts_;
      return;
    }
    ...
    uint64_t status = 0;
    if (!Envoy::StringUtil::atoul(response->Status()->value().c_str(), status)) {
      ENVOY_LOG(error, "Connection({}:{}) received response with bad status", connection.name(), connection.id());
  ...
  connect_callback_ = [this](ClientConnection &connection, ClientConnectionState state) -> ClientCallbackResult {
    ...
    if (0 >= requests_remaining_--) {
      // This will result in a ConnectionState::IDLE passed to this callback once all active streams have finished.
...
void LoadGenerator::run(uint32_t connections, uint32_t requests, Envoy::Http::HeaderMapPtr request,
                        const std::chrono::milliseconds timeout) {
  ...
  for (uint32_t i = 0; i < connections_to_initiate_; ++i) {
    client_.connect(socket_factory_, http_version_, address_, sockopts_, connect_callback_, close_callback_);
  }
