...
class ClientServerTest : public testing::Test, Envoy::Logger::Loggable<Envoy::Logger::Id::testing> {
 public:
  ClientServerTest()
      : transport_socket_factory_(),
        ip_version_(Envoy::Network::Address::IpVersion::v4),
        listening_socket_(Envoy::Network::Utility::parseInternetAddressAndPort(
                              fmt::format("{}:{}", Envoy::Network::Test::getAnyAddressUrlString(ip_version_), 0)),
                          nullptr, true),
        client_("client"),
        server_("server", listening_socket_, transport_socket_factory_, Envoy::Http::CodecClient::Type::HTTP1) {}
...
TEST_F(ClientServerTest, HappyPath) {
  ...
  Envoy::Network::Address::InstanceConstSharedPtr address = listening_socket_.localAddress();
  LoadGenerator load_generator(client_, transport_socket_factory_, HttpVersion::HTTP1, address);
...
TEST_F(ClientServerTest, AcceptAndClose) {
  ...
  // Assert that all connections succeed but no responses are received and the server closes the connections.
  EXPECT_EQ(load_generator.connectSuccesses(), connections_to_initiate);
  EXPECT_EQ(0, load_generator.connectFailures());
...
TEST_F(ClientServerTest, SlowResponse) {
  ...
  ServerCallbackHelper server_callbacks([](ServerConnection &, ServerStream &stream, Envoy::Http::HeaderMapPtr &&) {
  ...
  Envoy::Http::HeaderMapPtr request{new Envoy::Http::TestHeaderMapImpl{
      {":method", "GET"}, {":path", "/"}, {":scheme", "http"}, {":authority", "host"}}};
  load_generator.run(connections_to_initiate, requests_to_send, std::move(request), std::chrono::milliseconds(250));
  ...
  // Assert that all connections succeed but all responses timeout leading to local closing of all connections.
  EXPECT_EQ(load_generator.connectSuccesses(), connections_to_initiate);
  EXPECT_EQ(0, load_generator.connectFailures());
...
TEST_F(ClientServerTest, NoServer) {
  ...
  // Create a listening socket bound to an ephemeral port picked by the kernel, but don't create a server to call
  // listen() on it.  Result will be ECONNREFUSEDs and we won't accidentally send connects to another process.

  Envoy::Network::TcpListenSocket listening_socket(
      Envoy::Network::Utility::parseInternetAddressAndPort(
          fmt::format("{}:{}", Envoy::Network::Test::getAnyAddressUrlString(ip_version_), 0)),
      nullptr, true);
  uint16_t port = static_cast<uint16_t>(listening_socket.localAddress()->ip()->port());
