...
void setupCelFilterState(Envoy::StreamInfo::StreamInfo& stream_info) {
  ...
  auto cel_state = std::make_unique<Filters::Common::Expr::CelState>(FilterConfig::peerInfoPrototype());
  cel_state->setValue(absl::string_view(proto->SerializeAsString()));

  stream_info.filterState()->setData(
      std::string(Istio::Common::DownstreamPeer),
      std::move(cel_state),
...
static void BM_FilterState_CEL(benchmark::State& state) {
  ...
  ScopedThreadLocalServerContextSetter server_context_setter(
