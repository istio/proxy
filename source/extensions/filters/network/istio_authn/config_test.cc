...
TEST(Principal, Basic) {
  const std::string value1 =
      "spiffe://cluster.local/ns/my-namespace/sa/my-account1";
  ...
  const std::string value2 =
      "spiffe://cluster.local/ns/my-namespace/sa/my-account2";
...
TEST(IstioAuthnFilter, CallbacksWithSsl) {
  ...
  EXPECT_CALL(*ssl, uriSanLocalCertificate())
      .WillRepeatedly(Return(local_sans));
  callbacks.connection_.raiseEvent(Network::ConnectionEvent::Connected);
  const auto info =
      getPrincipals(*callbacks.connection_.stream_info_.filter_state_);
  EXPECT_EQ(info.peer, peer);
  EXPECT_EQ(info.local, local);
}
...
TEST(IstioAuthnFilter, CallbacksWithSslMultipleSAN) {
  ...
  auto ssl = std::make_shared<Ssl::MockConnectionInfo>();
  const std::string spiffe1 =
      "spiffe://cluster.local/ns/my-namespace/sa/my-account1";
  const std::string spiffe2 =
      "spiffe://cluster.local/ns/my-namespace/sa/my-account2";
